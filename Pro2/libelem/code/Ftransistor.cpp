...
void Ftransistor::exp_to_txt(std::ofstream &file)
{
    file<<ID<<" ";
    file<<width<<" ";
...
void Ftransistor::imp_from_txt(std::ifstream &file)
{
    file>>width;
    for(int i=0; i<3; i+=1)file>>line_color[i];
    for(int i=0; i<3; i+=1)file>>interior_color[i];
...
Ftransistor::Ftransistor(unsigned short width, unsigned short line_color[], unsigned short interior_color[], unsigned short stroke_width, unsigned short local[], string name)
{
	Element::width=width;
...
void Ftransistor::write_in_terminal()//wypisuje specyfikacje tego Ftransistoru w terminalu
{
	...
	cout<<"localization :"<<Element::local[0]<<" "<<Element::local[1]<<endl<<endl;
	#undef PRINT
    cout<<endl;
}

void Ftransistor::exp_to_svg(ofstream &file)
{
//kolo
